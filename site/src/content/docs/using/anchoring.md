...
listed in the sidebar, flagged, decorating nothing. The search is exact. Edit
or delete the text a comment was written on and the sidebar flags it, rather
than moving the highlight to whichever passage looks closest.
...
A resolved comment is never flagged as detached. It usually detaches because
its suggestion was applied, or because the passage was rewritten in answer to
it. It keeps its card and its quote,
without the dashed border or the alert, and stays at the bottom of the list.

## Edits elsewhere in the note

A comment is found by its quote. The position recorded beside it orders the
sidebar and breaks a tie when the same quote appears twice, and nothing keeps
it current. An edit anywhere else in the note leaves every comment attached,
...
grays out then, rather than flickering while you are still working on the
sentence.
